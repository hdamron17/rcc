...
impl<B: Backend> Compiler<B> {
    ...
    fn binary_assign_op(
        ...
        Self::binary_assign_ir(left, right, ctype, token, &self.target, builder)
    }
    fn binary_assign_ir(
        left: Value,
        right: Value,
    ...
    fn assignment(
        ...
        if token != AssignmentToken::Equal {
            ...
            let ir_type = ctype.as_ir_type(&self.target);
            let target = Value {
                ir_val: builder
                    .ins()
            ...
            FuncCall::Named(func_name) => {
                ...
                if ftype.varargs {
                    ...
                    let abi_params = ftype
                        .params
                        .into_iter()
                        .map(|param| AbiParam::new(param.ctype.as_ir_type(&self.target)))
